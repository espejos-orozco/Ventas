...
// Model for Sucursales entity
    models.Sucursales = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/VentasOnlinev2/webresources/entities.sucursales/",
        idAttribute: 'sucursalId',
        defaults: {
            alias: "",
            activo: ""
        },
        toViewJson: function () {
            ...
            result.displayName = this.get('sucursalId');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/VentasOnlinev2/webresources/entities.sucursales/';
            }
    ...
    // Collection class for Sucursales entities
    models.SucursalesCollection = Backbone.Collection.extend({
        model: models.Sucursales,
        url: "http://localhost:8080/VentasOnlinev2/webresources/entities.sucursales/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.SucursalesCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-sucursales-list-itemis template identifier for item
                        templateName: '#tpl-sucursales-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.sucursales = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.sucursales,
                    // tpl-sucursales-details is template identifier for chosen model element
                    templateName: '#tpl-sucursales-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Sucursales();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-sucursales-details is a template identifier for chosen model element
                templateName: '#tpl-sucursales-details',
