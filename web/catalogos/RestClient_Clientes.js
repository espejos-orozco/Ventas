...
// Model for Clientes entity
    models.Clientes = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/VentasOnlinev2/webresources/entities.clientes/",
        idAttribute: 'clienteId',
        defaults: {
            apellidos: "",
            password: "",
            telefono: "",
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('clienteId');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/VentasOnlinev2/webresources/entities.clientes/';
            }
    ...
    // Collection class for Clientes entities
    models.ClientesCollection = Backbone.Collection.extend({
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.ClientesCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-clientes-list-itemis template identifier for item
                        templateName: '#tpl-clientes-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.clientes = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.clientes,
                    // tpl-clientes-details is template identifier for chosen model element
                    templateName: '#tpl-clientes-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Clientes();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-clientes-details is a template identifier for chosen model element
                templateName: '#tpl-clientes-details',
