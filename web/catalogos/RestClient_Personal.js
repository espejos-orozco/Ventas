...
// Model for Personal entity
    models.Personal = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/VentasOnlinev2/webresources/entities.personal/",
        idAttribute: 'personalId',
        defaults: {
            accesoSistema: "",
            password: "",
            apellido: "",
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('personalId');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/VentasOnlinev2/webresources/entities.personal/';
            }
    ...
    // Collection class for Personal entities
    models.PersonalCollection = Backbone.Collection.extend({
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.PersonalCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-personal-list-itemis template identifier for item
                        templateName: '#tpl-personal-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.personal = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.personal,
                    // tpl-personal-details is template identifier for chosen model element
                    templateName: '#tpl-personal-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Personal();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-personal-details is a template identifier for chosen model element
                templateName: '#tpl-personal-details',
        ...
        getData: function () {
            return {
                ...
                accesoSistema: $('#accesoSistema').val(),
